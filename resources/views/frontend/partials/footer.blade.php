<footer class="border-t border-brand-teal-dark bg-brand-teal text-white">
    <div class="container mx-auto px-4 py-16">
        <div class="grid gap-12 md:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
                ...
                <ul class="mt-4 space-y-3">
                    ...
                    <li><a href="{{ route('blog.index') }}" class="text-sm text-white/90 hover:text-brand-gold transition-colors">{{ __('Blog') }}</a></li>
                </ul>
            </div>
            <div>
                ...
                <ul class="mt-4 space-y-3 text-sm text-white/90">
                    ...
                    @if(site_email())
                        <li class="flex items-center gap-2">
                            ...
                            <a href="mailto:{{ site_email() }}" class="hover:text-brand-gold transition-colors">{{ site_email() }}</a>
                        </li>
                    @endif
                    @if(site_email_secondary())
                        <li class="flex items-center gap-2">
                            <span class="material-icons text-lg text-brand-gold/80">email</span>
                            <a href="mailto:{{ site_email_secondary() }}" class="hover:text-brand-gold transition-colors">{{ site_email_secondary() }}</a>
                        </li>
                    @endif
                    @if(site_phone())
                        <li class="flex items-center gap-2">
                            <span class="material-icons text-lg text-brand-gold/80">phone</span>
                            <a href="tel:{{ preg_replace('/\s+/', '', site_phone()) }}" class="hover:text-brand-gold transition-colors">{{ site_phone() }}</a>
                        </li>
                    @endif
                    @if(site_phone_secondary())
                        <li class="flex items-center gap-2">
                            <span class="material-icons text-lg text-brand-gold/80">phone</span>
                            <a href="tel:{{ preg_replace('/\s+/', '', site_phone_secondary()) }}" class="hover:text-brand-gold transition-colors">{{ site_phone_secondary() }}</a>
                        </li>
                    @endif
                    @if(!site_location() && !site_email() && !site_email_secondary() && !site_phone() && !site_phone_secondary())
                        <li class="flex items-center gap-2"><span class="material-icons text-lg text-brand-gold/80">contact_support</span> <a href="{{ route('contact') }}" class="hover:text-brand-gold transition-colors">{{ __('Contact Us') }}</a></li>
                    @endif
                </ul>
                @php $hasSocial = site_whatsapp_url() || site_instagram_url() || site_facebook_url() || site_twitter_url(); @endphp
                @if($hasSocial)
                    <div class="mt-4 flex flex-wrap gap-2">
                        @if(site_whatsapp_url())
                            <a href="{{ site_whatsapp_url() }}" target="_blank" rel="noopener noreferrer" class="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-white transition hover:bg-green-500 hover:scale-110" aria-label="{{ __('WhatsApp') }}">
                        ...
                        @if(site_instagram_url())
                            <a href="{{ site_instagram_url() }}" target="_blank" rel="noopener noreferrer" class="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-white transition hover:bg-gradient-to-br hover:from-purple-500 hover:via-pink-500 hover:to-amber-500 hover:scale-110" aria-label="{{ __('Instagram') }}">
                        ...
                        @if(site_facebook_url())
                            <a href="{{ site_facebook_url() }}" target="_blank" rel="noopener noreferrer" class="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-white transition hover:bg-[#1877f2] hover:scale-110" aria-label="{{ __('Facebook') }}">
                                <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                            </a>
                        @endif
                        @if(site_twitter_url())
                            <a href="{{ site_twitter_url() }}" target="_blank" rel="noopener noreferrer" class="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-white transition hover:bg-[#1da1f2] hover:scale-110" aria-label="{{ __('Twitter') }}">
        ...
        @php $badges = array_filter(trust_badges()); @endphp
        @if(!empty($badges))
