...
@section('title', __('Contact Us') . ' – ' . site_title())
...
@section('content')
    ...
    {{-- Contact info from settings (location, email, phone, social) --}}
    @if(site_location() || site_email() || site_phone() || site_whatsapp_url() || site_instagram_url() || site_facebook_url() || site_twitter_url())
    <section class="border-b border-slate-200 bg-brand-white py-10 md:py-12">
        <div class="container mx-auto px-4">
            <h2 class="text-center text-xl font-bold text-brand-black md:text-2xl">{{ __('Get in touch') }}</h2>
            <div class="mx-auto mt-8 flex max-w-4xl flex-wrap justify-center gap-8">
                @if(site_location())
                    <div class="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50/50 px-6 py-4">
                        ...
                        <div>
                            <p class="text-sm font-medium text-brand-black/70">{{ __('Address') }}</p>
                            <p class="text-brand-black">{{ site_location() }}</p>
                        </div>
                    </div>
                @endif
                @if(site_email())
                    <a href="mailto:{{ site_email() }}" class="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50/50 px-6 py-4 transition hover:border-brand-teal/30 hover:bg-brand-gold/5">
                        <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-brand-teal/10 text-brand-teal">
                            <span class="material-icons text-xl">email</span>
                        </span>
                        <div>
                            <p class="text-sm font-medium text-brand-black/70">{{ __('Email') }}</p>
                            <p class="text-brand-teal">{{ site_email() }}</p>
                        </div>
                    </a>
                @endif
                @if(site_phone())
                    <a href="tel:{{ preg_replace('/\s+/', '', site_phone()) }}" class="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50/50 px-6 py-4 transition hover:border-brand-teal/30 hover:bg-brand-gold/5">
                        <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-brand-teal/10 text-brand-teal">
                            <span class="material-icons text-xl">phone</span>
                        </span>
                        <div>
                            <p class="text-sm font-medium text-brand-black/70">{{ __('Phone') }}</p>
                            <p class="text-brand-teal">{{ site_phone() }}</p>
                        </div>
                    </a>
                @endif
                @if(site_whatsapp_url() || site_instagram_url() || site_facebook_url() || site_twitter_url())
                    <div class="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50/50 px-6 py-4">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @if(site_whatsapp_url())
                                <a href="{{ site_whatsapp_url() }}" target="_blank" rel="noopener noreferrer" class="flex h-10 w-10 items-center justify-center rounded-full bg-green-500 text-white transition hover:scale-110" aria-label="{{ __('WhatsApp') }}">
                            ...
                            @if(site_instagram_url())
                                <a href="{{ site_instagram_url() }}" target="_blank" rel="noopener noreferrer" class="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-purple-500 via-pink-500 to-amber-500 text-white transition hover:scale-110" aria-label="{{ __('Instagram') }}">
                            ...
                            @if(site_facebook_url())
                                <a href="{{ site_facebook_url() }}" target="_blank" rel="noopener noreferrer" class="flex h-10 w-10 items-center justify-center rounded-full bg-[#1877f2] text-white transition hover:scale-110" aria-label="{{ __('Facebook') }}">
                                    <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                                </a>
                            @endif
                            @if(site_twitter_url())
                                <a href="{{ site_twitter_url() }}" target="_blank" rel="noopener noreferrer" class="flex h-10 w-10 items-center justify-center rounded-full bg-[#1da1f2] text-white transition hover:scale-110" aria-label="{{ __('Twitter') }}">
