...
@component('admin::components.card', ['title' => null])
    ...
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <a href="{{ route('admin.posts.create') }}" class="inline-flex items-center gap-1.5 rounded-lg bg-brand-teal px-4 py-2 text-sm font-medium text-white transition hover:bg-brand-teal-dark">
                ...
                @forelse($posts as $post)
                    <tr class="hover:bg-slate-50/70">
                        ...
                        <td class="px-4 py-3 text-right">
                            ...
                            <a href="{{ route('admin.posts.edit', $post) }}" class="inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium text-brand-teal transition hover:bg-brand-gold/10">{{ __('Edit') }}</a>
                            <form action="{{ route('admin.posts.destroy', $post) }}" method="POST" class="inline-block" onsubmit="return confirm('{{ __('Are you sure you want to delete this post?') }}');">
                                @csrf
                                @method('DELETE')
