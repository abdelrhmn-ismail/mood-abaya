        ...
        <div class="flex min-w-0 flex-1 flex-col">
            <header class="flex h-16 flex-shrink-0 items-center justify-between gap-4 border-b border-slate-200 bg-brand-white px-4 md:px-6">
                <div class="flex items-center gap-3">
                    <button type="button" id="sidebar-toggle" class="rounded-lg p-2 text-brand-teal hover:bg-brand-gold/10 md:hidden" aria-label="<?php echo e(__('Open menu')); ?>">
                        <span class="material-icons">menu</span>
                    </button>
                    <?php if(!$__env->hasSection('heading') || trim((string) $__env->yieldContent('heading')) !== ''): ?>
                        <h1 class="truncate text-lg font-semibold text-brand-black"><?php echo $__env->yieldContent('heading', __('Admin')); ?></h1>
                    <?php endif; ?>
                </div>
                <div class="flex items-center gap-3">
                    <a href="<?php echo e(url('/')); ?>" class="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-medium text-brand-black transition hover:bg-brand-gold/10 hover:text-brand-teal" target="_blank">
                        <span class="material-icons text-lg">storefront</span> <?php echo e(__('View store')); ?>

                    </a>
                    <div class="relative">
                        <label for="admin-locale" class="sr-only"><?php echo e(__('Language')); ?></label>
                        <select id="admin-locale" class="appearance-none rounded-xl border border-slate-200 bg-slate-50 py-2 text-sm font-medium text-brand-black focus:border-brand-teal focus:outline-none focus:ring-2 focus:ring-brand-teal/20 <?php echo e(app()->getLocale() === 'ar' ? 'pl-8 pr-3' : 'pl-3 pr-8'); ?>" onchange="window.location.href='<?php echo e(url('/locale')); ?>/'+this.value">
                        ...
                        <span class="pointer-events-none absolute top-1/2 -translate-y-1/2 text-slate-400 <?php echo e(app()->getLocale() === 'ar' ? 'left-2' : 'right-2'); ?>">
                            <span class="material-icons text-lg">arrow_drop_down</span>
                        </span>
                    </div>
                    <form method="POST" action="<?php echo e(route('logout')); ?>" class="inline">
                        <?php echo csrf_field(); ?>
                        <button type="submit" class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-medium text-brand-black transition hover:bg-brand-gold/10 hover:text-brand-teal"><?php echo e(__('Logout')); ?></button>
                    </form>
                </div>
            </header>
