<?php $__env->startSection('title', __('Blog') . ' – ' . config('app.name')); ?>
<?php $__env->startSection('description', __('News and updates from') . ' ' . config('app.name')); ?>
                    ...
                    <?php $__currentLoopData = $posts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $post): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <article class="flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-brand-white shadow-sm transition hover:shadow-lg">
                            <a href="<?php echo e(route('blog.show', $post->slug)); ?>" class="block aspect-video w-full overflow-hidden bg-slate-100">
                                <?php if($post->image): ?>
                                    <img src="<?php echo e(asset('storage/' . $post->image)); ?>" alt="<?php echo e($post->title); ?>" class="h-full w-full object-cover transition duration-300 hover:scale-105" loading="lazy">
                                <?php else: ?>
